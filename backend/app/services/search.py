...
def search_articles(
    ...
    vector_distance = ArticleChunk.embedding.l2_distance(query_embedding)

    vector_cte = (
        select(
            ArticleChunk.article_id.label("article_id"),
    ...
    text_rank = func.ts_rank(ArticleChunk.search_vector, fts_query).desc()
    ...
    fts_cte = (
        ...
        .where(ArticleChunk.search_vector.op("@@")(fts_query))
        .order_by(text_rank)
        .limit(top_k * 2)
